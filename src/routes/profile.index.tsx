...
import { HeartPulse, Target, Settings, LogOut, ChevronRight, Camera, PhoneCall } from "lucide-react";
    ...
    <Screen noHeader bottomNav contentClass="px-5 pb-6">
      <div className="pt-6 text-center">
        <div className="relative mx-auto h-24 w-24">
          <div
            className="flex h-full w-full items-center justify-center rounded-full text-3xl font-bold text-primary-foreground shadow-[var(--shadow-float)]"
            style={{ background: "var(--gradient-primary)" }}
          >
            {userName.slice(0, 2).toUpperCase()}
          </div>
          <button className="absolute bottom-0 right-0 flex h-8 w-8 items-center justify-center rounded-full bg-card shadow-[var(--shadow-soft)]">
      ...
      <button
        onClick={logout}
        className="mt-6 flex w-full items-center gap-4 rounded-2xl bg-card p-4 shadow-[var(--shadow-soft)]"
      >
